...
```{r, include = F}
knitr::opts_chunk$set(eval = F, collapse = T, comment = NA)
```
...
If your have distinct data sets for the outcome (Y), covariate matrix (Z), and provider indicator (ID), you can directly execute the subsequent code to do the data preparation.
```{r}
...
### 2.3 Testing to find the outlier facilities
```{r}
test_fe(fit)
```
