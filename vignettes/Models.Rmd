...
* **"exact.poisbinom"**: Assume $Y_{ij}|Z_{ij} \sim Bernoulli(p_{ij})$. Since within the facility $i$, $p_{ij} = \frac{e^{\gamma_i + Z_{ij}\boldsymbol{\beta}}}{1 + e^{\gamma_i + Z_{ij}\boldsymbol{\beta}}}$ varies, $O_i | Z_i = \sum_{j = 1}^{n_i}Y_{ij}$ will follow the Poisson-binomial distribution. We then calculate the probability of "getting a more extreme number of $O_i$, if $H_0$ is true" based on the Poisson-binomial distribution.
...
* $DSR_i$: Considering $DSR_{0} = \frac{\sum_{i = 1}^m \sum_{j = 1}^{n_i} \frac{e^{{\gamma}_0 + Z_{ij}\boldsymbol{\beta}}}{1 + e^{{\gamma}_0 + Z_{ij}\boldsymbol{\beta}}}}{\sum_{i = 1}^m O_i}.$ The CI of $DSR_i$ can be calculated as:

$$CI_{DSR_i} = (\frac{\sum_{i = 1}^m \sum_{j = 1}^{n_i} \frac{e^{{\gamma}_{i,L} + Z_{ij}\boldsymbol{\beta}}}{1 + e^{{\gamma}_{i,L} + Z_{ij}\boldsymbol{\beta}}}}{\sum_{i = 1}^m O_i}, \frac{\sum_{i = 1}^m \sum_{j = 1}^{n_i} \frac{e^{{\gamma}_{i,U} + Z_{ij}\boldsymbol{\beta}}}{1 + e^{{\gamma}_{i,U} + Z_{ij}\boldsymbol{\beta}}}}{\sum_{i = 1}^m O_i}).$$ *(If $CI(\gamma_i)$ covers $\gamma_0$, then $CI_{DSR_i}$ will cover $DSR_0 = \frac{\sum_{i = 1}^m \sum_{j = 1}^{n_i} \frac{e^{{\gamma}_0 + Z_{ij}\boldsymbol{\beta}}}{1 + e^{{\gamma}_0 + Z_{ij}\boldsymbol{\beta}}}}{\sum_{i = 1}^m O_i}$.)*
